...
export const sendOtp = async (req, res) => {
  try {
    ...
    const otpExpiry = Date.now() + process.env.OTP_TTL_MINUTES * 60 * 1000; // 5 minutes from now
...
export const verifyOtp = async (req, res) => {
  ...
  const token = jwt.sign({ mobile }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '7d' });
  res.json({
    success: true,
